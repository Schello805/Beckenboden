...
apt-get update
apt-get install -y git curl ca-certificates rsync openssl gnupg sqlite3
...
if [[ ! -f /etc/mein-kraftbaum.env ]]; then
  ...
  printf 'SESSION_SECRET=%s\nINSTALL_TOKEN=%s\nDATA_DIR=%s\nAPP_REVISION=%s\nAPP_URL=%s\n' "${SESSION_SECRET}" "${INSTALL_TOKEN}" "${APP_DIR}/data" "0.16.0" "${APP_URL:-http://localhost:3000}" > /etc/mein-kraftbaum.env
...
echo "NPM Plus auf http://$(hostname -I | awk '{print $1}'):3000 weiterleiten."
