const VERSION="staerke-deine-mitte-v0349";
...
self.addEventListener("notificationclick",event=>{event.notification.close();const clear="clearAppBadge" in self.navigator?self.navigator.clearAppBadge():Promise.resolve();event.waitUntil(Promise.all([clear,clients.matchAll({type:"window",includeUncontrolled:true}).then(windows=>{const existing=windows.find(client=>"focus" in client);return existing?existing.focus():clients.openWindow(event.notification.data.url||"/")})]))});
