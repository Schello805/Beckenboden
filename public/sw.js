...
self.addEventListener("push",event=>{const data=event.data?.json()||{title:"Mein Kraftbaum",body:"Es gibt etwas Neues für dich.",url:"/"};event.waitUntil(self.registration.showNotification(data.title,{body:data.body,icon:"/favicon.svg",badge:"/favicon.svg",data:{url:data.url||"/"}}))});
