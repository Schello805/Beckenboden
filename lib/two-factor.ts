...
export function newTotp(email:string){const secret=new OTPAuth.Secret({size:20}),totp=new OTPAuth.TOTP({issuer:"Mein Kraftbaum",label:email,algorithm:"SHA1",digits:6,period:30,secret});return {secret:secret.base32,uri:totp.toString()}}
export function validTotp(secret:string,token:string){const totp=new OTPAuth.TOTP({issuer:"Mein Kraftbaum",algorithm:"SHA1",digits:6,period:30,secret:OTPAuth.Secret.fromBase32(secret)});return totp.validate({token,window:1})!==null}
