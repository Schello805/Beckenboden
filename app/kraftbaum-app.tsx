...
export function KraftbaumApp() {
  ...
  return <div className="app"><Header isAdmin={user.role==="admin"} onAdmin={()=>setView(user.role==="admin"?"admin":"profil")}/><div className="desktop-tabs">{nav.map(n=><button className={view===n.id?"active":""} onClick={()=>setView(n.id)} key={n.id}>{n.label}</button>)}</div>{view==="baum"&&<BaumView setView={setView}/>} {view==="kurse"&&<KurseView/>}{view==="termine"&&<TermineView/>}{view==="nuetzliches"&&<UsefulView/>}{view==="profil"&&<ProfileView/>}<footer><div><a href="https://anja-tanzt.de/impressum/">Impressum</a><a href="https://anja-tanzt.de/datenschutzerklaerung/">Datenschutz</a><a href="https://anja-tanzt.de/agb/">Nutzungsbedingungen</a><button type="button">Cookie-Einstellungen</button></div><span>Mein Kraftbaum · Revision 0.2.1</span></footer><nav className="mobile-nav">{nav.map(n=><button className={view===n.id?"active":""} onClick={()=>setView(n.id)} key={n.id}><i>{n.icon}</i>{n.label}</button>)}</nav></div>;
}
