...
function TreeScene({ progress = 3, courses = 1, completed = 0, figureMediaId = null }: { progress?: number; courses?:number; completed?:number;figureMediaId?:string|null }) {
...
function BaumView({ setView,data }: { setView: (v: View) => void;data:ReturnType<typeof useDashboard> }) {
  ...
  return <>
    <section className="hero">
      ...
      <TreeScene progress={attended} courses={data?.courses.length||1} completed={completed} figureMediaId={data?.appearance?.figureMediaId}/>
...
function AccessScreen({ setupRequired, onSuccess }:{setupRequired:boolean;onSuccess:(user:AuthUser)=>void}){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);setError("");const form=new FormData(event.currentTarget);const body=Object.fromEntries(form.entries());const endpoint=mode==="setup"?"/api/setup/initialize":mode==="login"?"/api/auth/login":"/api/auth/register";const response=await fetch(endpoint,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});const result=await response.json().catch(()=>({error:"Unbekannter Fehler."}));setBusy(false);if(result.requiresTwoFactor){setNeedsTwoFactor(true);setTwoFactorMethod(result.twoFactorMethod||"totp");if(!response.ok)setError(result.error||"");return}if(!response.ok){setError(result.error||"Das hat leider nicht geklappt.");return}const me=await fetch("/api/me").then(r=>r.json());onSuccess(me.user);}
