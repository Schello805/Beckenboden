...
export async function POST(request:Request){
  ...
  if(!user||user.status!=="active"||!await bcrypt.compare(parsed.data.password,user.passwordHash)) return Response.json({error:"E-Mail oder Passwort ist falsch."},{status:401});
