...
export async function GET(){const admin=await requireAdmin();if(!admin)return Response.json({error:"Nicht berechtigt."},{status:403});let updateStatus:unknown=null;try{updateStatus=JSON.parse(await fs.readFile(path.join(dataDir,"update-status.json"),"utf8"))}catch{}let available=false,remoteRevision:string|null=null;try{const {stdout}=await exec("git",["ls-remote","origin","refs/heads/main"],{cwd:process.cwd(),timeout:5000});const remote=stdout.trim().split(/\s/)[0],{stdout:local}=await exec("git",["rev-parse","HEAD"],{cwd:process.cwd(),timeout:2000});available=Boolean(remote&&remote!==local.trim());remoteRevision=remote.slice(0,7)}catch{}return Response.json({currentRevision:process.env.APP_REVISION||"development",available,remoteRevision,updateStatus})}
