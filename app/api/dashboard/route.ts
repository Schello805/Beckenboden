...
export async function GET(){
  ...
  const upcoming=db.prepare(`SELECT s.id,s.course_id courseId,s.sequence,s.title,s.starts_at startsAt,s.ends_at endsAt,COALESCE(s.location,c.location) location,COALESCE(s.navigation_url,c.navigation_url) navigationUrl FROM course_sessions s JOIN courses c ON c.id=s.course_id JOIN enrollments e ON e.course_id=c.id WHERE e.user_id=? AND s.ends_at>=? ORDER BY s.starts_at LIMIT 20`).all(user.id,new Date().toISOString());
  const missed=db.prepare(`SELECT s.id,s.course_id courseId,s.sequence,s.title,s.starts_at startsAt,s.ends_at endsAt,COALESCE(s.location,c.location) location,COALESCE(s.navigation_url,c.navigation_url) navigationUrl FROM course_sessions s JOIN courses c ON c.id=s.course_id JOIN enrollments e ON e.course_id=c.id LEFT JOIN attendance a ON a.session_id=s.id AND a.user_id=e.user_id WHERE e.user_id=? AND e.completed_at IS NULL AND s.ends_at<? AND a.id IS NULL ORDER BY s.starts_at DESC LIMIT 10`).all(user.id,new Date().toISOString());
  ...
  return Response.json({user,courses,upcoming,missed,appearance,decorations});
}
