...
export async function POST(request:Request){if(!allowed(request))return Response.json({error:"Nicht berechtigt."},{status:403});const mail=await processMailQueue(30),health=await queueSystemAlerts();return Response.json({mail,health},{headers:{"cache-control":"no-store"}})}
