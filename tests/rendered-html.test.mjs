...
test("composes the start page as a refined responsive growth scene",async()=>{
  ...
  assert.match(app,/className="hero-progress"/);
  assert.ok(app.indexOf('className="hero-progress"')<app.indexOf("<TreeScene progress={attended}"),"Fortschritt muss vor dem Baum stehen");
  ...
  assert.match(styles,/\.hero-progress\{position:absolute/);
...
test("lets admins send and permanently record manual push messages by course or globally",async()=>{
  const [admin,collection,database,audit,styles]=await Promise.all([
  ...
  assert.match(admin,/dauerhaft und kann nicht verändert oder gelöscht werden/);
  assert.doesNotMatch(admin,/push-messages\/\$\{message\.id\}/);
