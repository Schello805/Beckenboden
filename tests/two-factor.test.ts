...
test("accepts a current authenticator token",()=>{const setup=newTotp("admin@example.de"),totp=new OTPAuth.TOTP({issuer:"Mein Kraftbaum",algorithm:"SHA1",digits:6,period:30,secret:OTPAuth.Secret.fromBase32(setup.secret)});assert.equal(validTotp(setup.secret,totp.generate()),true);assert.equal(validTotp(setup.secret,"000000"),false)});
